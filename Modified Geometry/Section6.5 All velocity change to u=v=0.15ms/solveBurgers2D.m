...
for i=2:m+1
    if xv(i)>=0.875&&xv(i)<=1.125
            Dv2(1,i-1)=d1*vs(i+1,2)+(1-2*d1)*vs(i,2)+d1*vs(i-1,2)+0.5*dt*Sv(i,2)+0.15*d2;
...
for i=2:m+1
    if xv(i)>=1.875&&xv(i)<=2.125
                Dv2(end,i-1)=d1*vs(i+1,end-1)+(1-2*d1)*vs(i,end-1)+d1*vs(i-1,end-1)+0.5*dt*Sv(i,end-1)-0.15*d2;
